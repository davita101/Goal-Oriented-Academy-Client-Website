  ...
  return (
    <div className='p-4 absolute top-0 w-full bg-background overflow-hidden z-[1] pt-20 '>
      <div className=''>
        ...
        {/* //? profile up */}
        <div className="font-bold my-4 sm:text-xl text-sm capitalize flex items-center">
          <React.Fragment>
            <div className='  flex items-center'>
              {t("applications")}
              <ChevronRight size={20} className="mt-1.5" color="hsl(var(--primary))" />
            </div>
            <div className=' flex items-center'>
              {t("profile Edit")}
              <ChevronRight size={20} className="mt-1.5" color="hsl(var(--primary))" />
      ...
      <div className=''>
        ...
        >
          <ResizablePanel defaultSize={15} maxSize={15} >
            <div>
              <Avatar className=" w-42 h-42 rounded-none">
            ...
            <ul className='flex flex-col items-start'>
              {accountEditArr.map((item, index) => (
                <>
                  <li key={`settings-${item}`} className='flex flex-col max-sm:hidden items-start '>
                  ...
                  <li key={`settings-${item}`} className='flex flex-col sm:hidden items-start '>
                    <Link to={`/applications/profile-edit/${item}`} className=' hover:bg-stone-100 rounded-md w-56'>
